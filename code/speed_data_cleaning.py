...
# DATA_FOLDER = '../data/speed_debug/'
DATA_FOLDER = '../data/speed_raw/'
SAVE_FOLDER = '../data/data_processed/'
PARSE_KEYWORDS = ['All Eastbound', 'All Westbound', 'All Northbound', 'All Southbound']
PROCESS_ALL = True  # Set to True to process all files, even if the output folder already exists
...
def parse_table(table, keywords=None):
    ...
    for keyword in keywords:
        ...
        for index in keyword_indices:
            ...
            extracted_df = df.iloc[index + 2:index + 28]
...
def parse_html_file(file_path):
    ...
    for table in tables:
        ...
        if any(keyword in table.text for keyword in PARSE_KEYWORDS):
            ...
            os.makedirs(os.path.join(SAVE_FOLDER, site_name, 'speed'), exist_ok=True)
            for keyword, df_parsed in df_dict.items():
                output_path = os.path.join(SAVE_FOLDER, site_name, 'speed', f'{site_name}_{date}_{keyword}.csv')
...
def main():
    ...
    for site_folder in os.listdir(DATA_FOLDER):
        ...
        if not PROCESS_ALL and os.path.exists(os.path.join(SAVE_FOLDER, site_folder, 'speed')):
